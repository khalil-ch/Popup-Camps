...
require_once '../../config.php';
      ...
      session_start();
        if (isset($_GET['commande']) && isset($_POST['commander']) && (isset($_SESSION['id'])) ){
            $commandeC = new commandeC();
            $commandeItemsC = new commandeItemsC();
            $panierC = new panierC();
           
           $id_commande = $commandeC->ajouterCommande($_SESSION['id']); // $_SESSION['id']
            $id_commande = $id_commande['MAX(id_commande)'];
           // var_dump($id_commande);
            $liste = $panierC->afficherPanier($_SESSION['id']); // $_SESSION['id']
          ...
          /*  $query = "select sum(qt*prix) from(select q.qt_produit as qt , p.prix_produit as prix from commande_items q inner Join
            ...
            $req->execute(
                ...
                $panierC->supprimerTout($_SESSION['id']); //session
